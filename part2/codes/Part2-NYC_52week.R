...
p <- plot_ly(data, x = c(xaxis)) %>%
  ...
  layout(xaxis = list(type = 'category',title = "Week"),yaxis = list(range = c(0,15000),title = 'Number of positive specimens'),
         yaxis2 = list(range = c(0,1500),overlaying = 'y',side = 'right', title = "Percent Positive"), 
         barmode = 'overlay', title='National Summary for NY State, 2018-2019 Season')
p
